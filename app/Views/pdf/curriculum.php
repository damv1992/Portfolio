...
<center><h1 style="color: <?=$color?>;"><u>CURRICULUM VITAE</u></h1></center>
  <div class="resumen">
    <div class="izquierda">
      <div class="texto">
    ...
    <div class="derecha">
      <div class="texto">
        <h2 style="color: <?=$color?>;"><u>PERFIL PROFESIONAL</u></h2>
        <p><?=$acerca['Descripcion']?></p>
        <?php if (count($experienciass)) { ?><h2 style="color: <?=$color?>;"><u>EXPERIENCIA LABORAL</u></h2><?php } ?>
        <ul>
  ...
  <div>
    <?php if (count($educacioness)) { ?><h2 style="color: <?=$color?>;"><u>FORMACION ACADEMICA</u></h2><?php } ?>
    <ul>
    ...
    <?php if (count($cursoss)) { ?><h2 style="color: <?=$color?>;"><u>CURSOS</u></h2><?php } ?>
    <ul>
      <?php foreach ($cursoss as $curso) { ?>
      <li><strong><?=$curso['Curso']?></strong></li>
      <?=mb_strtoupper($curso['Institucion'])?>
      <?php
        $inicio = date('d/m/Y', strtotime($curso['FechaInicio']));
